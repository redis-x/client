...
function input$72(key, value, options) {
	const args_options = [];
	if (options) {
...
*/
function input$69(key, increment) {
	return {
		kind: "#schema",
		args: [
			"INCRBYFLOAT",
...
*/
function input$64(key, offset, value) {
	return {
		kind: "#schema",
		args: [
			"SETRANGE",
...
*/
function input$63(key, start, end) {
	return {
		kind: "#schema",
		args: [
			"SUBSTR",
...
*/
function input$61(key, decrement) {
	return {
		kind: "#schema",
		args: [
			"DECRBY",
...
*/
function input$60(key, start, end) {
	return {
		kind: "#schema",
		args: [
			"GETRANGE",
...
*/
function input$59(key, seconds, value) {
	return {
		kind: "#schema",
		args: [
			"SETEX",
...
*/
function input$55(key, milliseconds, value) {
	return {
		kind: "#schema",
		args: [
			"PSETEX",
...
*/
function input$54(key, seconds, options) {
	const args_options = [];
	if (options) {
		if (options.NX) args_options.push("NX");
		if (options.XX) args_options.push("XX");
...
* @see {@link https://redis.io/commands/expire}
*/
function input$51(key, seconds, options) {
	const args_options = [];
	if (options) {
		if (options.NX) args_options.push("NX");
		if (options.XX) args_options.push("XX");
...
*/
function input$48(source, destination, options) {
	const args = [
		"COPY",
		source,
		destination
...
* @see {@link https://redis.io/commands/expire}
*/
function input$47(key, timestamp, options) {
	const args_options = [];
	if (options) {
		if (options.NX) args_options.push("NX");
		if (options.XX) args_options.push("XX");
...
* @see {@link https://redis.io/commands/expireat}
*/
function input$46(key, timestamp, options) {
	const args_options = [];
	if (options) {
		if (options.NX) args_options.push("NX");
		if (options.XX) args_options.push("XX");
...
*/
function input$36(source, destination) {
	return {
		kind: "#schema",
		args: [
			"RPOPLPUSH",
...
function input$35(key, ...elements) {
	return {
		kind: "#schema",
		args: [
			"RPUSHX",
...
*/
function input$34(key, index, element) {
	return {
		kind: "#schema",
		args: [
			"LSET",
...
*/
function input$33(key, count, element) {
	return {
		kind: "#schema",
		args: [
			"LREM",
...
function input$32(key, ...elements) {
	return {
		kind: "#schema",
		args: [
			"LPUSH",
...
*/
function input$29(key, start, stop) {
	return {
		kind: "#schema",
		args: [
			"LRANGE",
...
*/
function input$27(key, element, options) {
	return {
		kind: "#schema",
		args: [
			"LINSERT",
...
*/
function input$26(key, start, stop) {
	return {
		kind: "#schema",
		args: [
			"LTRIM",
...
var RedisXTransactionUse = class {
	...
	*/
	SMISMEMBER(key, ...members) {
		return this.useCommand(input$80(key, ...members));
	}
	/**
	* Returns all the members of the set value stored at key.
	*
	...
	*/
	SMEMBERS(key) {
		return this.useCommand(input$79(key));
	}
	/**
	* Returns if member is a member of the set stored at key.
	*
	...
	*/
	SISMEMBER(key, member) {
		return this.useCommand(input$78(key, member));
	}
	/**
	* Returns the set cardinality (number of elements) of the set stored at key.
	*
	...
	*/
	MSETNX(pairs) {
		return this.useCommand(input$75(pairs));
	}
	/**
	* Set key to hold string value if key does not exist. In that case, it is equal to SET.
	* When key already holds a value, no operation is performed. SETNX is short for "SET if Not eXists".
	...
	*/
	SETNX(key, value) {
		return this.useCommand(input$74(key, value));
	}
	/**
	* Get the value of key.
	*
	...
	*/
	...
	SET(key, value, options) {
		return this.useCommand(input$72(key, value, options));
	}
	/**
	* Append a value to a key.
	*
	...
	*/
	APPEND(key, value) {
		return this.useCommand(input$71(key, value));
	}
	/**
	* Increments the number stored at key by one. If the key does not exist,
	* it is set to 0 before performing the operation. An error is returned if the
	...
	*/
	INCR(key) {
		return this.useCommand(input$70(key));
	}
	/**
	* Increment the string representing a floating point number stored at key by the specified increment.
	* By using a negative increment value, the result is that the value stored at the key is decremented.
	...
	*/
	INCRBYFLOAT(key, increment) {
		return this.useCommand(input$69(key, increment));
	}
	MGET(...args) {
		return this.useCommand(input$68(...args));
	}
	/**
	* Get the value of key and delete the key. This command is similar to GET, except for the fact
	* that it also deletes the key on success (if and only if the key's value type is a string).
	...
	*/
	GETDEL(key) {
		return this.useCommand(input$67(key));
	}
	/**
	* Sets the given keys to their respective values. MSET replaces existing values with new values, just as regular SET.
	* MSET is atomic, so all given keys are set at once. It is not possible for clients to see that some of the keys were updated while others are unchanged.
	...
	*/
	MSET(pairs) {
		return this.useCommand(input$66(pairs));
	}
	/**
	* Increments the number stored at key by increment. If the key does not exist, it is set to 0 before performing the operation.
	* An error is returned if the key contains a value of the wrong type or contains a string that cannot be represented as integer.
	...
	*/
	SETRANGE(key, offset, value) {
		return this.useCommand(input$64(key, offset, value));
	}
	/**
	* Returns the substring of the string value stored at key, determined by the offsets start and end (both are inclusive).
	* Negative offsets can be used in order to provide an offset starting from the end of the string.
	...
	*/
	SUBSTR(key, start, end) {
		return this.useCommand(input$63(key, start, end));
	}
	/**
	* Atomically sets key to value and returns the old value stored at key.
	* Returns an error when key exists but does not hold a string value.
	...
	*/
	GETSET(key, value) {
		return this.useCommand(input$62(key, value));
	}
	/**
	* Reduces the value stored at the specified key by the specified decrement.
	* If the key does not exist, it is initialized with a value of 0 before performing the operation.
	...
	*/
	DECRBY(key, decrement) {
		return this.useCommand(input$61(key, decrement));
	}
	/**
	* Returns the substring of the string value stored at key, determined by the offsets start and end (both are inclusive).
	* Negative offsets can be used in order to provide an offset starting from the end of the string.
	...
	*/
	GETRANGE(key, start, end) {
		return this.useCommand(input$60(key, start, end));
	}
	/**
	* Set key to hold the string value and set key to timeout after a given number of seconds.
	* This command is equivalent to SET key value EX seconds.
	...
	*/
	SETEX(key, seconds, value) {
		return this.useCommand(input$59(key, seconds, value));
	}
	/**
	* Get the value of key and optionally set its expiration.
	* GETEX is similar to GET, but is a write command with additional options.
	...
	*/
	GETEX(key, options) {
		return this.useCommand(input$58(key, options));
	}
	/**
	* Returns the length of the string value stored at key.
	* An error is returned when key holds a non-string value.
	...
	*/
	PSETEX(key, milliseconds, value) {
		return this.useCommand(input$55(key, milliseconds, value));
	}
	/**
	* This command works exactly like EXPIRE but the time to live of the key is specified in milliseconds instead of seconds.
	* - Available since: 2.6.0.
	...
	*/
	PEXPIRE(key, seconds, options) {
		return this.useCommand(input$54(key, seconds, options));
	}
	/**
	* Returns the remaining time to live of a key that has a timeout, in milliseconds.
	*
	...
	*/
	EXPIRE(key, seconds, options) {
		return this.useCommand(input$51(key, seconds, options));
	}
	/**
	* Returns the string representation of the type of the value stored at `key`.
	* - Available since: 1.0.0.
	...
	*/
	KEYS(pattern) {
		return this.useCommand(input$49(pattern));
	}
	/**
	* Copy the value stored at the source key to the destination key.
	* - Available since: 6.2.0.
	...
	*/
	COPY(source, destination, options) {
		return this.useCommand(input$48(source, destination, options));
	}
	/**
	* This command has the same effect and semantic as EXPIRE, but instead of specifying the number of seconds representing the TTL (time to live), it takes an absolute Unix timestamp (seconds since January 1, 1970).
	*
	...
	*/
	EXPIREAT(key, timestamp, options) {
		return this.useCommand(input$47(key, timestamp, options));
	}
	/**
	* This command has the same effect and semantic as EXPIREAT, but the Unix time at which the key will expire is specified in milliseconds instead of seconds.
	* - Available since: 2.6.0.
	...
	*/
	PEXPIREAT(key, timestamp, options) {
		return this.useCommand(input$46(key, timestamp, options));
	}
	/**
	* Remove the existing timeout on key, turning the key from volatile (a key with an expire set) to persistent (a key that will never expire as no timeout is associated).
	*
	...
	*/
	EXPIRETIME(key) {
		return this.useCommand(input$44(key));
	}
	/**
	* Renames `key` to `newkey`. It returns an error when `key` does not exist. If `newkey` already exists it is overwritten.
	*
	...
	*/
	RENAME(key, newkey) {
		return this.useCommand(input$43(key, newkey));
	}
	/**
	* Renames `key` to `newkey` if `newkey` does not yet exist. It returns an error when `key` does not exist.
	*
	...
	*/
	RENAMENX(key, newkey) {
		return this.useCommand(input$42(key, newkey));
	}
	/**
	* PEXPIRETIME has the same semantic as EXPIRETIME, but returns the absolute Unix expiration timestamp in milliseconds instead of seconds.
	* - Available since: 7.0.0.
	...
	*/
	DEL(...keys) {
		return this.useCommand(input$40(...keys));
	}
	/**
	* Returns the number of keys that exist from those specified as arguments.
	*
	...
	*/
	EXISTS(...keys) {
		return this.useCommand(input$39(...keys));
	}
	RPUSH(key, ...elements) {
	...
	*/
	RPOPLPUSH(source, destination) {
		return this.useCommand(input$36(source, destination));
	}
	RPUSHX(key, ...elements) {
		return this.useCommand(input$35(key, ...elements));
	}
	/**
	* Sets the list element at index to element. For more information on the index argument, see LINDEX.
	*
	...
	*/
	LSET(key, index, element) {
		return this.useCommand(input$34(key, index, element));
	}
	/**
	* Removes the first count occurrences of elements equal to element from the list stored at key.
	* The count argument influences the operation in the following ways:
	...
	*/
	LREM(key, count, element) {
		return this.useCommand(input$33(key, count, element));
	}
	LPUSH(key, ...elements) {
		return this.useCommand(input$32(key, ...elements));
	}
	LPUSHX(key, ...elements) {
		return this.useCommand(input$31(key, ...elements));
	}
	RPOP(key, count) {
		return this.useCommand(input$30(key, count));
	}
	/**
	* Returns the specified elements of the list stored at key. The offsets start and stop
	* are zero-based indexes, with 0 being the first element of the list (the head of the list),
	...
	*/
	LRANGE(key, start, stop) {
		return this.useCommand(input$29(key, start, stop));
	}
	/**
	* Returns the length of the list stored at key. If key does not exist, it is interpreted as an empty list and 0 is returned.
	* An error is returned when the value stored at key is not a list.
	...
	*/
	LINSERT(key, element, options) {
		return this.useCommand(input$27(key, element, options));
	}
	/**
	* Trim an existing list so that it will contain only the specified range of elements specified.
	* Both `start` and `stop` are zero-based indexes, where 0 is the first element of the list (the head),
	...
	*/
	LTRIM(key, start, stop) {
		return this.useCommand(input$26(key, start, stop));
	}
	/**
	* Returns the element at index in the list stored at key.
	* The index is zero-based, so 0 means the first element, 1 the second element and so on.
	...
	*/
	LINDEX(key, index) {
		return this.useCommand(input$25(key, index));
	}
	/**
	* Returns the sorted set cardinality (number of elements) of the sorted set stored at key.
	* - Available since: 1.2.0.
